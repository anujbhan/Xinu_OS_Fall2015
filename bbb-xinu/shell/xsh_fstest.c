 ...
 shellcmd xsh_fstest(int nargs, char *args[])
{
	...
	fd = fs_create("Test_File9", O_CREAT);
	fd = fs_open("Test_File9", O_RDWR);

	//fd = fs_create("Test_Fil9", O_CREAT);
    printf("file create with fd :%d\n", fd);
    ...
    printf("write success, fileptr :%d\n", rval);
    /*if(rval == 0 || rval != SIZE )
    {
        printf("\n\r File write failed");
        goto clean_up;
    ...
    seek_st = fs_seek(fd,-rval); 
    
    printf("File Pointer is in %d",seek_st);
